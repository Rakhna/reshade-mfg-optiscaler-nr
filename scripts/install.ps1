...
# Resolve dist directory from either repo root or scripts folder
if (Test-Path (Join-Path $PSScriptRoot "dist")) {
    $distDir = Join-Path $PSScriptRoot "dist"
} elseif (Test-Path (Join-Path $PSScriptRoot "..\dist")) {
    $distDir = (Resolve-Path (Join-Path $PSScriptRoot "..\dist")).Path
} else {
    Write-Error "Could not locate 'dist' folder."
    exit 1
}
...
# Deploy root proxies and canonical OptiScaler/ subfolder
...
Write-Host "  [+]   - Quick Toggle DLSS Neural Rendering On/Off"
Write-Host ""
